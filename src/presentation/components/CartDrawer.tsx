...
export function CartDrawer({ open, onClose }: CartDrawerProps) {
  ...
  return (
    <>
      ...
      />
      <aside
        className="fixed bottom-0 right-0 top-0 z-50 flex w-full max-w-md flex-col border-l border-white/10 bg-[#0f0c0a] shadow-2xl shadow-black/60"
        role="dialog"
        aria-modal="true"
        ...
        <div className="border-t border-white/10 p-5">
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              disabled={lines.length === 0}
              className="rounded-full bg-emerald-600 px-5 py-3.5 text-sm font-semibold text-white shadow-lg shadow-emerald-950/40 transition hover:bg-emerald-500 disabled:cursor-not-allowed disabled:bg-stone-700 disabled:text-stone-400 disabled:shadow-none"
            >
              {t('cart.send')}
            </button>
          </div>
